"""Plot GEFS ensemble agreement and wettest-member precipitation maps."""
...
DATA_FILE = Path("data/gefs_precip_ensemble.npz")
OUT_PROB = Path("output/GEFS_PRECIP_ENSEMBLE_AGREEMENT_0_72H_LATEST.png")
OUT_MAX = Path("output/GEFS_PRECIP_WETTEST_MEMBER_0_72H_LATEST.png")
EXTENT = [20, 65, 10, 45]
...
def main():
    ...
    init = np.datetime_as_string(data["init_time"].astype("datetime64[h]"), unit="h").replace("T", " ")
    agreement = np.mean(rain >= 25.0, axis=0) * 100.0

    OUT_PROB.parent.mkdir(exist_ok=True)
    fig, ax = base_ax(
        f"GEFS Ensemble Agreement: 0–72 h Rain ≥25 mm – Middle East\nInit: {init} UTC | 31 members"
    )
    shown = np.ma.masked_less(agreement, 10.0)
    plot = ax.contourf(lons, lats, shown, levels=np.arange(10, 101, 10),
    ...
    plt.figtext(0.5, 0.02, "Agreement map: percentage of GEFS members reaching the threshold", ha="center")
    ...
    fig, ax = base_ax(
        f"GEFS Wettest Member: {wettest_name} – 0–72 h Precipitation\nInit: {init} UTC"
    )
    shown = np.ma.masked_less(wettest, 1.0)
    levels = [1, 5, 10, 25, 50, 75, 100, 150, 200]
    plot = ax.contourf(lons, lats, shown, levels=levels, cmap="turbo",
                       extend="max", transform=ccrs.PlateCarree())
    cbar = plt.colorbar(plot, ax=ax, pad=0.025, shrink=0.82)
    cbar.set_label("Accumulated precipitation (mm)")
    plt.figtext(0.5, 0.02, "Extreme scenario, not the ensemble mean or deterministic forecast", ha="center")
