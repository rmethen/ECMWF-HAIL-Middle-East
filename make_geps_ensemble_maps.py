"""Plot Canadian GEPS rainfall agreement and wettest-member maps."""
...
DATA_FILE = Path("data/geps_precip_ensemble.npz")
OUT_PROB = Path("output/GEPS_PRECIP_AGREEMENT_0_72H_LATEST.png")
OUT_MAX = Path("output/GEPS_PRECIP_WETTEST_MEMBER_0_72H_LATEST.png")
EXTENT = [20, 65, 10, 45]
...
def main():
    ...
    member_count = rain.shape[0]
    agreement = np.mean(rain >= 25.0, axis=0) * 100.0

    OUT_PROB.parent.mkdir(exist_ok=True)
    fig, ax = base_ax(
        "Canadian GEPS Agreement: 0–72 h Rain ≥25 mm – Middle East\n"
        f"Init: {init} UTC | {member_count} members"
    )
    ...
    plt.figtext(0.5, 0.02,
                "Agreement map: percentage of Canadian GEPS members reaching the threshold",
                ha="center")
    plt.savefig(OUT_PROB, dpi=160, bbox_inches="tight")
    plt.close(fig)

    member_totals = np.nanmax(rain.reshape(member_count, -1), axis=1)
    wettest_idx = int(np.nanargmax(member_totals))
    wettest = rain[wettest_idx]
    wettest_name = str(members[wettest_idx])
    fig, ax = base_ax(
        f"Canadian GEPS Wettest Member: {wettest_name} – 0–72 h Precipitation\n"
        f"Init: {init} UTC"
    )
    ...
    plt.figtext(0.5, 0.02,
                "Extreme GEPS scenario, not the ensemble mean or deterministic forecast",
                ha="center")
